...
# Store.where(mens_apparel: true)
@mens_stores = Store.where(womens_apparel: true, annual_revenue: 0..999999)

@mens_stores.each do |store|
  puts store.name
end

@womens_stores = Store.where(mens_apparel: true, annual_revenue: 0..999999)
